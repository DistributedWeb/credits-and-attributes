...
* Must not be distributed under a licence specific to DWeb. The rights attached to the software must not depend on the           program being part of a DWeb-based system. So we will not distribute software for which DWeb has a "special" exemption         or right, and we will not put our own open-source Software into DWeb and then refuse you the right to pass it on. Although,   software CAN BE closed-source, just as many dWeb-based software is closed-source. 
